...
- Native activities: `curriculum.components` joined to assigned `curriculum.modules` and `curriculum.weeks`. Dates use the shared `read_builder_activity_dates()` calendar, matching My Learning: explicit session dates and trustworthy component/week titles take precedence; otherwise lessons inherit the planned delivery week from the module start date, delivery days, week order and selected cohort holidays. Empty weeks consume their planned slots. Introduction and ambiguous dates retain their existing placement.
...
- Only modules with activities scheduled in the current week appear in the selector. The first weekly module is the initial selection; creating or assigning a future module does not add a placeholder or change the focus.
- Refreshes preserve a manual selection while the learner and calendar week are unchanged and the selected module still has weekly activities. At the next UK Monday, the panel moves to the new week regardless of unfinished work. Past unfinished activities remain in the full learning plan; they do not hold this card on a past week. An empty week shows the empty state without falling back to past or future modules.
...
Weekly selection verification (13 September 2026): 22 database-free overview tests and 110 frontend tests passed, covering future-module exclusion, multiple weekly modules, retained selection, rescheduling, empty weeks, UK Monday rollover with unfinished work, and learner-page integration. The shared Builder-date suite passed 8 of 9 tests; its existing `test_selected_holidays_move_components_into_the_correct_month` fails because it expects reading/assignment dates to shift for a holiday while the planner retains their original week. The same failure was reproduced independently with the reader, planner and test unchanged from HEAD.

The full TypeScript check reports four errors outside this change: the missing `slotDate` type in `curriculum/module-workspace/page.tsx` and three unsupported `exact` options in the existing untracked `learner/video-watch/completion.test.tsx`.
...
- Live sessions in both panels are scoped to the selected weekly module's exact curriculum ID(s), including explicit old/new mappings. New sessions appear on refresh. There is no fallback to another module's sessions. Coaching/review events remain learner-wide.
