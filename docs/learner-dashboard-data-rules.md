...
عند غياب إجمالي صالح في الوثيقة، يُحسب Target من مجموع الساعات المخططة لكل الأنشطة المعيّنة للطالب عبر البرنامج كله. تُستخدم `curriculum.components.expected_otjh` للأنشطة الحالية و`Last_audit.activity_planned_hours.planned_hours` للأنشطة القديمة، مع احتساب النشاط المرتبط صراحةً بين المصدرين مرة واحدة. إجمالي الوثيقة الصالح، بما فيه صفر، يحتفظ بالأولوية. إذا كانت ساعات بعض الأنشطة مفقودة فلا يُعرض مجموع جزئي كأنه إجمالي البرنامج.
...
من 2026-09-13، يتيح مسار التجميع من الأنشطة أعلاه عرض Target عند غياب إجمالي الوثيقة. يعتمد على الأنشطة المعيّنة فعليًا ويُعاد حسابه عند القراءة؛ إتمام نشاط أو تكرار محاولة لا يضيف إلى الساعات المخططة. لا يحتاج هذا الإصلاح إلى أي تغيير في قاعدة البيانات.
