...
لعلاج خطأ النسخ من SharePoint، راجع [TRANSFER_FIX_EN_GB.md](TRANSFER_FIX_EN_GB.md). أُضيفت أداة `upload_material_file.py` لرفع الملف بعد تنزيله والتحقق من النسخة على Azure، دون أي تغيير في قاعدة البيانات.
