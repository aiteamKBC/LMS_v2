...
- **Restrictions:** no agent commits, pushes, pulls, PRs, migrations, or database changes; preserve unrelated work; Teams checks are required.
...
## 3. Database and external-system restrictions

- Do not use database migrations. Do not create, run, or modify migration files or migration commands.
- Do not execute database-changing SQL or make database changes through application tooling, management commands, HTTP requests, browser interactions, or connectors.
- If a schema or data change is needed, provide the exact SQL in the response, with purpose, affected records/tables, and a read-only verification query. The project owner executes it manually in the Neon DB SQL Editor.
- Do not use a schema-repair helper, seed script, test runner, or an automatically invoked `ensure_*` function to bypass these rules.
- The word "test", a localhost URL, a branch name, or `--keepdb` does not establish safety or permission. Inspect test setup and side effects before execution. No automatic exception for temporary/test databases is granted by this policy.
- Do not start a server or run backend/browser tests against an unverified database configuration. Do not print connection strings or secrets while checking configuration.
- Use mocked external services for routine regression tests. Do not create, reschedule, cancel, or delete real Teams meetings, send invitations/emails, change tenant settings, upload/delete real evidence, or call paid services as a side effect of testing.
- A request to edit integration code does not authorize live operations. Live validation needs a separately specified test environment and explicit authorization for its external effects; database restrictions still apply.
- Never change `.env` files, credentials, permissions, or deployment configuration just to make tests pass.
...
The existing `backend/login/test_runner.py` provisions schema/data, and several Django suites create or mutate tables/fixtures. Do not run a blanket `manage.py test`, a branch wrapper, or a migration-based setup under the standing database restriction. A no-database case must be verified to avoid database setup, live connections, and external side effects, including during imports.

If no compliant execution path exists, list the affected suites as blocked and hand the owner the prerequisites for an owner-run test environment. Do not invent a passing backend result or relax the restriction. Live Teams smoke checks also remain separate and require the authorization described in section 3.
...
Outstanding: <failures, missing environment, owner-run SQL, or review needed>
```
