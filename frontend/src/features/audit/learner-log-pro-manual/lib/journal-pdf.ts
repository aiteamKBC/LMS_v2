...
type JournalSummary = {
  ...
  // True when the figure is an auditor-entered override rather than the
  // computed aggregate — surfaced on the PDF metrics as "(manual entry)".
...
export async function downloadLearnerJournalPdf(
  ...
  drawMetric(doc, 12, learner.planned_hours_overridden ? "Monthly plan (manual entry)" : "Monthly plan", hours(learner.planned_hours), navy);
  drawMetric(doc, 81.5, learner.actual_hours_overridden ? "Claimed (manual entry)" : "Claimed", hours(learner.actual_hours), purple);
