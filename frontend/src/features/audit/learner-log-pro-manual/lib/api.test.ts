...
describe("audit-copy cohort overlay accounting", () => {
  ...
  it("keeps the Activities counter in sync with manual adds, deletes, and edits", () => {
    const withCounts = { learners: [{ ...cohort.learners[0], activity_count: 5 }] };
    const overlay = (patch: Record<string, unknown>) => [{
      aptem_id: 92, payload: activity(), source_payload: null,
      updated_by: null, updated_at: null, ...patch,
    }] as never;

    const created = applyCohortOverlay(withCounts as never, overlay({
      activity_id: "audit:1", operation: "created", payload: activity({ activity_id: "audit:1" }),
    }));
    expect(created.learners[0].activity_count).toBe(6);

    const deletedLive = applyCohortOverlay(withCounts as never, overlay({
      activity_id: "source:1", operation: "deleted",
    }));
    expect(deletedLive.learners[0].activity_count).toBe(4);

    // Deleting an audit-created row replaces its own "created" record, so the
    // tombstone alone must not change the count.
    const deletedAudit = applyCohortOverlay(withCounts as never, overlay({
      activity_id: "audit:1", operation: "deleted", payload: activity({ activity_id: "audit:1" }),
    }));
    expect(deletedAudit.learners[0].activity_count).toBe(5);

    const replaced = applyCohortOverlay(withCounts as never, overlay({
      activity_id: "source:1", operation: "replaced", source_payload: activity(),
    }));
    expect(replaced.learners[0].activity_count).toBe(5);
  });
