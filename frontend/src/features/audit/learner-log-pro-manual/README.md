# MANUAL audit workspace (learner-log-pro-manual)

> This is the MANUAL audit system — an independent clone of the automatic
> audit workspace. Frontend: this folder. Backend: `backend/manual_audit_api`.
> Database: the `Manual_audit` schema (a synced mirror of `Last_audit` plus
> manual-owned write tables). It shares no endpoints or tables with the
> automatic system, so changes here never affect `/workspace/auditor-copy`.
>
> Reads:  `/manual_audit_api/ledger/*` (cohort / activities / activity /
>         quiz-attempt / attendance-sheet)
> Writes: `/manual_audit_api/match-ledger/*` (overlays, annotations,
>         learner-profile), `/manual_audit_api/contracts|evidence/*`,
>         `/manual_audit_api/learners/<id>/signoff/`
> Sync:   `python manage.py setup_manual_audit` (recreates the mirror from
>         Last_audit; manual work in the write tables is never touched).

The original auditor-copy README is kept below for reference.
