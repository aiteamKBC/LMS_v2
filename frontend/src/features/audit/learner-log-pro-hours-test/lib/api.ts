...
// The auditor-hours-test workspace now reads normalized learner/activity data from
// the LMS Django API backed by the `Last_audit` schema. The read endpoints are:
//
//   GET /hours_test_api/last-audit/cohort/               -> all matched LMS learners
//   GET /hours_test_api/last-audit/activities/?aptem_id= -> one learner's normalized results
//   GET /hours_test_api/last-audit/activity/?activity_id= -> shared definition + participants
//
...
// service is gone.
const READ_BASE = "/hours_test_api/last-audit";
...
const ANNOTATION_BASE = "/hours_test_api/match-ledger";
...
function fetchLedgerTotals(): Promise<Map<number, LedgerTotals>> {
  ...
  ledgerTotalsPromise = fetch("/hours_test_api/last-audit/manual/cohort-totals")
    .then(async (response) => {
...
export async function uploadContract(learnerId: number, file: File) {
  ...
  const response = await fetch("/hours_test_api/contracts/upload", {
    method: "POST",
    body,
  });
  if (!response.ok) {
...
export async function setContractArchived(contractId: string, archived: boolean) {
  const response = await fetch(`/hours_test_api/contracts/${encodeURIComponent(contractId)}/archive`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function renameContract(contractId: string, documentName: string) {
  const response = await fetch(`/hours_test_api/contracts/${encodeURIComponent(contractId)}/name`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function uploadEvidence(learnerId: number, file: File, evidenceDate: string, componentName?: string) {
  ...
  body.append("file", file);
  const response = await fetch("/hours_test_api/evidence/upload", {
    method: "POST",
    body,
  });
  if (!response.ok) {
...
export async function selectActivityEvidence(input: {
  ...
  const response = await fetch("/hours_test_api/evidence/select-activity", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
export async function updateEvidenceDate(evidenceId: string, learnerId: number, evidenceDate: string) {
  const response = await fetch(`/hours_test_api/evidence/${encodeURIComponent(evidenceId)}/date`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function setEvidenceArchived(evidenceId: string, learnerId: number, archived: boolean) {
  const response = await fetch(`/hours_test_api/evidence/${encodeURIComponent(evidenceId)}/archive`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
