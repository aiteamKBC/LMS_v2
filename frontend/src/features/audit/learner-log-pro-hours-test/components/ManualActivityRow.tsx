...
import { DurationInput, formatHoursDuration } from "@/features/audit/learner-log-pro-hours-test/components/DurationInput";
import { TableCell, TableRow } from "@/features/audit/learner-log-pro-hours-test/components/ui/table";
import { durationAsHours, formatDurationMinutes } from "@/features/audit/learner-log-pro-hours-test/lib/manualApi";
import type { DraftPatch, DraftRow } from "@/features/audit/learner-log-pro-hours-test/lib/journalDraft";
import {
  dateRestriction,
  isUkBankHoliday,
  isUkWeekend,
  ...
  WORK_DAY_START,
  workingTimeRange,
} from "@/features/audit/learner-log-pro-hours-test/lib/ukCalendar";
...
export function ManualActivityRow({ row, onPatch, onDelete, onStageFiles, onUnstageFile, onDeleteDocument, mergeMode = false, mergeEligible = false, mergeSelected = false, onToggleMerge, className = "", reportMonth, dateWindow, calculatedHours }: {
  row: DraftRow;
      ...
      <TableRow className={className}>
        ...
        <TableCell className="whitespace-nowrap text-right font-mono text-sm text-success">
          ...
          {calculatedHours ? (
            <span
              className="mt-0.5 block rounded-sm border border-dashed border-sky-300 bg-sky-50 px-1 text-[11px] font-semibold text-sky-800"
              title={`Calculated from the ${calculatedHours.basis} — pending approval by another auditor`}
            >
