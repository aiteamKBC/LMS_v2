# HOURS-TEST OTJH workspace — a clone of the Automatic workspace

> **What this directory is.** A standalone copy of
> `learner-log-pro-copy` (the "Automatic" audit system), reached from
> `/workspace/audit` → **HOURS-TEST** at `/workspace/auditor-hours-test`.
>
> Two things make it independent of Automatic:
> 1. **Its own code.** Nothing here is imported by `learner-log-pro-copy`, and
>    its CSS is scoped to `.learner-log-pro-hours-test`, so redesigning this
>    workspace cannot change the live one.
> 2. **Its own database.** Every request goes to `/hours_test_api/…` instead of
>    `/audit_api/…`. Both prefixes run the *same* Django views; the HOURS-TEST
>    mount (`audit_api/clone_urls.py`) resolves the `audit` and `enrolment`
>    aliases to the `audit_clone` connection — the cloned Neon branch configured
>    as `LASR-ADUTIOD-CLNE` in `backend/.env`. Hours edited here never reach the
>    live audit data.
>
> Shared on purpose: Azure blob storage (evidence/contract files) and the live
> `KBCDATABASE` attendance register, both read-only source data.

The rest of this file is the inherited Automatic-workspace documentation.

> Current state (2026-08-12): everything is same-origin Django. Reads come from
> `/hours_test_api/last-audit/` (normalized `"Last_audit"` schema). The **monthly
...
> rows employees add via `/hours_test_api/last-audit/manual/*` (schema
...
Last_audit.learners (580 Aptem identities; optional learner_id = LMS id)
    ...
    -> GET /hours_test_api/last-audit/cohort/

Last_audit.activity_results + activities
Last_audit.learner_attendance (linked from AiTeamKBC.kbc_attendance by Aptem ID)
    -> GET /hours_test_api/last-audit/activities/?aptem_id=<id>
    -> GET /hours_test_api/last-audit/activity/?component_id=la:<group>:<activity>
    -> GET /hours_test_api/last-audit/activity/?component_id=att:<source_key>
```
...
This workspace (`/workspace/auditor-hours-test`) shows PLANNED vs ACTUAL off-the-job
...
- **Row edits on the read-only mirror** — reversible replacement overlays via
  `/hours_test_api/match-ledger/activity-overrides` (`updateActivityRow` /
  `deleteActivityRow` / `createActivity` in `lib/api.ts`).
- **Employee-arranged journal rows** — CRUD on
  `/hours_test_api/last-audit/manual/rows`, assignment uploads on
  `/hours_test_api/last-audit/manual/documents` (`lib/manualApi.ts`), stored in the
...
These stay pointed at the existing Django `/hours_test_api` endpoints:

- **Activity annotations** (auditor planned-hours override + notes) —
  `getActivityAnnotation` / `saveActivityAnnotation` → `/hours_test_api/match-ledger/…`.
- **Monthly sign-off** (learner + coach signatures) → `/hours_test_api/learners/<id>/signoff/`
