...
Clicking an **attendance** record in the auditor-hours-test workspace should open an
...
Base path (same-origin, proxied to Django): **`/hours_test_api/last-audit`**
(this is `READ_BASE` in `lib/api.ts`). All are `GET`, read-only, no auth token.

### 1.1 `GET /hours_test_api/last-audit/attendance-sheet/` — NEW (built)
...
|----------|---------|----------------|
| `GET /hours_test_api/last-audit/activities/?aptem_id=<id>` | one learner's full activity feed (the monthly journal). Attendance rows here already carry `activity_id = "att:{source_key}"` and `source_activity_id = "{source_key}"`. | This is where the **click originates**: the attendance row's `plan_id`/`activity_id` already contains the exact key to pass to §1.1 — no client-side key assembly needed. |
| `GET /hours_test_api/last-audit/activity/?component_id=<ref>` | one activity across all learners ("click an activity"). | Sibling pattern; the attendance sheet is the attendance equivalent. |
| `GET /hours_test_api/last-audit/cohort/` | learner list for the workspace. | Unrelated, listed for context. |

> There is also a legacy `GET /hours_test_api/match-ledger/attendance-session?key=…`
...
Feature dir: `src/features/audit/learner-log-pro-hours-test/`.
Router: **TanStack Router**, file-based, **hash history** (URLs live in the hash,
e.g. `…/auditor-hours-test#/attendance?key=…`).
...
```ts
// READ_BASE = "/hours_test_api/last-audit"
...
| layer | file | state |
|-------|------|-------|
| backend view + pure builder | `backend/hours_test_api/last_audit_ledger_views.py` (`attendance_sheet`, `_attendance_sheet_payload`, `_session_key`) | **built** |
| backend route | `backend/hours_test_api/urls.py` (`last-audit/attendance-sheet/`) | **built** |
| backend test | `backend/hours_test_api/test_last_audit_ledger_views.py` | **built** |
| fe api wrapper | `frontend/src/features/audit/learner-log-pro-hours-test/lib/api.ts` (`getAttendanceSheet`) | to build |
