/**
 * Report what a person does in Curriculum Studio, for the Audit Trail's People
 * view.
 *
 * The change trail is written by the backend's own save helpers, so it already
 * knows every edit. What it cannot know is the reading: somebody opening the
 * module builder, searching a cohort and leaving touches no record, so nothing
 * anywhere records that it happened. That half has to be reported by the
 * browser, and this is the only place that does it.
 *
 ...
 *    and the record id in a URL are all resolved server-side. What goes up is
 *    the route, the kind of action, and short descriptive detail.
 ...
 *    anything on screen.
 *
 * Scope is Curriculum Studio: a path outside `/curriculum` is ignored here, so
 * a shared component calling `recordCurriculumAction` from another workspace
 * reports nothing rather than silently widening the tracking surface.
 */
...
const RECORD_PATH = '/curriculum/activity/record/';

const VISIT_KEY = 'kbc_curriculum_visit';
...
export interface CurriculumActivityDetail {
  query?: string;
  filter?: string;
  value?: string;
  tab?: string;
...
interface QueuedEvent {
  ...
  targetLabel?: string;
  durationMs?: number;
  detail?: CurriculumActivityDetail;
}
...
function isCurriculumPath(path: string): boolean {
  return path === '/curriculum' || path.startsWith('/curriculum/');
}
 ...
 */
export function flushCurriculumActivity(useBeacon = false): void {
  if (flushTimer !== null) {
...
/**
 * A curriculum page was opened.
 *
 * Called once per navigation from the router. The arrival is sent immediately;
 * when the page is left, the same path is sent again carrying how long it was
 * open, which the backend uses to CLOSE the arrival rather than to add a second
 * entry. A tab closed without warning simply never sends the second one, and
 * the page keeps an honest "opened, duration unknown" instead of a guess.
 */
export function recordCurriculumPageView(path: string, targetLabel = ''): void {
  if (typeof window === 'undefined') return;
  const clean = String(path || '').split('?')[0].split('#')[0];
  if (!isCurriculumPath(clean)) {
    // Left the curriculum: close the page that was open, then stop.
    closeCurrentPage();
    flushCurriculumActivity();
    return;
  }
 ...
 */
export function recordCurriculumAction(
  kind: Exclude<CurriculumActivityKind, 'page_view'>,
  detail: CurriculumActivityDetail = {},
  target: { type?: string; id?: string; label?: string } = {},
): void {
  if (typeof window === 'undefined') return;
  const path = currentPath || (typeof location !== 'undefined' ? location.pathname : '');
  if (!isCurriculumPath(path)) return;
  enqueue({
    kind,
    path,
    at: new Date().toISOString(),
...
export function recordCurriculumSearch(query: string, scope = 'search'): void {
  ...
  searchTimers.set(scope, setTimeout(() => {
    ...
    recordCurriculumAction('search', { query: text, scope });
  }, SEARCH_SETTLE_MS));
}

/** Test seam: forget everything this module is holding. */
export function resetCurriculumActivity(): void {
  queue = [];
