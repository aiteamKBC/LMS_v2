      ...
      <main className="w-full space-y-5 p-4 md:p-6">
        {loading ? <Loading /> : error ? <ErrorState message={error} /> : !attendance ? <EmptyState /> : <>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(300px,.6fr)]">
            <section className="rounded-3xl border border-background-200 bg-white p-5 shadow-[0_5px_24px_rgba(28,10,55,0.05)] md:p-6"><div className="flex items-center justify-between"><div><h2 className="text-base font-bold text-foreground-900">Attendance breakdown</h2><p className="mt-1 text-xs text-foreground-400">Present and absent sessions from your current record</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${risk.soft}`}>{risk.label}</span></div><div className="mt-7"><div className="flex h-4 overflow-hidden rounded-full bg-background-100"><div className="bg-emerald-500 transition-all" style={{ width: `${presentWidth}%` }}></div><div className="bg-[#ff0000] transition-all" style={{ width: `${absentWidth}%` }}></div></div><div className="mt-3 flex flex-wrap gap-5 text-xs"><span className="flex items-center gap-2 text-foreground-600"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500"></span><strong>{attendance.present}</strong> present</span><span className="flex items-center gap-2 text-foreground-600"><span className="h-2.5 w-2.5 rounded-full bg-[#ff0000]"></span><strong>{attendance.absent}</strong> absent</span><span className="flex items-center gap-2 text-foreground-600"><span className="h-2.5 w-2.5 rounded-full bg-amber-400"></span><strong>{attendance.late}</strong> late arrivals</span></div></div><div className="mt-7 grid gap-3 sm:grid-cols-2"><InfoBox icon="ri-calendar-check-line" label="Last session" value={formatDate(attendance.lastSessionDate)} /><InfoBox icon="ri-refresh-line" label="Record updated" value={formatDate(attendance.updatedAt, true)} /></div></section>
