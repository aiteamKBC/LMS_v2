        ...
        {loading ? <Loading /> : error ? <ErrorState message={error} /> : !attendance ? <EmptyState /> : <>
          ...
          <div className="flex items-start gap-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sky-800">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-sky-100">
              <i className="ri-microsoft-line text-base"></i>
            </span>
            <div>
...
function Loading() { return <div className="rounded-3xl border border-background-200 bg-white p-16 text-center text-sm text-foreground-400"><i className="ri-loader-4-line mr-2 animate-spin text-primary-600"></i>Loading verified Teams attendance…</div>; }
...
function EmptyState() { return <div className="rounded-3xl border border-dashed border-foreground-300 bg-white px-6 py-16 text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 text-primary-500"><i className="ri-calendar-check-line text-xl"></i></span><h2 className="mt-3 text-sm font-bold text-foreground-800">No verified attendance yet</h2><p className="mt-1 text-xs text-foreground-400">Attendance will appear after a completed live session has been synced with its Microsoft Teams attendance report.</p></div>; }
