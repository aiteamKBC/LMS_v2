...
function EventRow({ event, booking, busy, onBook, onCancel }: { event: EngagementEvent; booking?: EventBooking; busy: boolean; onBook: () => void; onCancel: () => void }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-foreground-200/70 bg-white shadow-[0_5px_24px_rgba(28,10,55,0.05)] transition-all duration-300 hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-[0_14px_36px_rgba(68,30,115,0.11)]">
      ...
      <div className="grid gap-4 p-4 pl-5 sm:gap-5 sm:p-5 sm:pl-6 lg:grid-cols-[82px_minmax(0,1fr)_minmax(250px,0.7fr)_130px] lg:items-center">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 lg:flex-col lg:items-stretch">
          ...
          {booking ? <div className="flex w-full items-center gap-2 lg:flex-col"><span className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-50 px-3 text-xs font-bold text-emerald-700 ring-1 ring-inset ring-emerald-200"><i className="ri-checkbox-circle-fill"></i>Booked</span><button onClick={onCancel} disabled={busy} className="text-[10px] font-semibold text-foreground-400 transition hover:text-red-600 disabled:opacity-50">{busy ? 'Updating…' : 'Cancel booking'}</button></div> : <button onClick={onBook} disabled={busy || event.status === 'completed'} className="h-10 w-full rounded-xl bg-primary-700 px-4 text-xs font-bold text-white shadow-md shadow-primary-700/20 transition hover:bg-primary-800 disabled:cursor-not-allowed disabled:bg-background-200 disabled:text-foreground-400 disabled:shadow-none lg:w-auto">{busy ? <i className="ri-loader-4-line animate-spin"></i> : event.status === 'completed' ? 'Booking closed' : 'Book place'}</button>}
        </div>
      </div>
    </article>
  );
      ...
      <main className="w-full space-y-4 p-3 sm:p-4 md:space-y-5 md:p-6">
        <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-4 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] sm:rounded-3xl sm:p-6 md:p-7">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_500px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-secondary-100"><i className="ri-calendar-event-line text-secondary-300"></i>Community calendar</span><h1 className="mt-3 text-[22px] font-bold leading-tight text-white sm:text-2xl md:text-3xl">Connect beyond the classroom</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/70">Find workshops, networking sessions and community events, with all the details you need in one place.</p></div>
        ...
        <section className="rounded-2xl border border-foreground-200/70 bg-white/95 p-2.5 shadow-[0_8px_30px_rgba(31,14,59,0.08)] backdrop-blur-xl md:sticky md:top-2 md:z-10">
          <div className="flex flex-col gap-2.5 xl:flex-row xl:items-center xl:justify-between"><div className="grid grid-cols-2 gap-1.5 sm:flex">{(['all', 'upcoming', 'ongoing', 'completed'] as StatusFilter[]).map((item) => { const count = item === 'all' ? events.length : item === 'upcoming' ? counts.upcoming : item === 'ongoing' ? counts.ongoing : counts.completed; return <button key={item} onClick={() => setStatus(item)} className={`flex min-w-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl px-2.5 py-2.5 text-[11px] font-semibold transition sm:gap-2 sm:px-4 sm:text-xs ${status === item ? 'bg-primary-700 text-white shadow-md shadow-primary-700/20' : 'text-foreground-500 hover:bg-primary-50 hover:text-primary-700'}`}><span className="truncate">{item === 'all' ? 'All events' : statusStyle[item].label}</span><span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] ${status === item ? 'bg-white/15 text-white' : 'bg-background-100 text-foreground-400'}`}>{count}</span></button>; })}</div><div className="flex flex-col gap-2 sm:flex-row"><select value={type} onChange={(event) => setType(event.target.value)} className="h-10 w-full rounded-xl border border-foreground-200 bg-background-50 px-3 text-xs font-semibold text-foreground-600 outline-none focus:border-primary-400 sm:w-auto"><option value="all">All event types</option>{types.map((item) => <option key={item} value={item}>{item[0].toUpperCase() + item.slice(1)}</option>)}</select><label className="relative block sm:w-72"><i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-foreground-400"></i><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search events…" className="h-10 w-full rounded-xl border border-foreground-200 bg-background-50 pl-9 pr-3 text-xs outline-none transition focus:border-primary-400 focus:bg-white focus:ring-4 focus:ring-primary-100" /></label></div></div>
        </section>
...
function Loading() { return <div className="rounded-2xl border border-background-200 bg-white px-4 py-10 text-center text-sm text-foreground-400 sm:rounded-3xl sm:p-14"><i className="ri-loader-4-line mr-2 animate-spin text-primary-600"></i>Loading events from the database…</div>; }
function Empty() { return <div className="rounded-2xl border border-dashed border-foreground-300 bg-white px-4 py-10 text-center sm:rounded-3xl sm:px-6 sm:py-16"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 text-primary-500"><i className="ri-calendar-close-line text-xl"></i></span><h2 className="mt-3 text-sm font-bold text-foreground-800">No events found</h2><p className="mt-1 text-xs leading-5 text-foreground-400">Try changing the filters, or check back when new events are published.</p></div>; }
