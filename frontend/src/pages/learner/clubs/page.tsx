      ...
      <main className="w-full space-y-4 p-3 sm:p-4 md:space-y-5 md:p-6">
        <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-4 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] sm:rounded-3xl sm:p-6 md:p-7">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_480px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-secondary-100"><i className="ri-community-line text-secondary-300"></i>Learner community</span><h1 className="mt-3 text-[22px] font-bold leading-tight text-white sm:text-2xl md:text-3xl">Find your community at KBC</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/70">Explore active clubs, meet other learners and keep up with community meetings and events.</p><div className="mt-4 flex flex-wrap gap-2 sm:mt-5"><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80"><i className="ri-checkbox-circle-line mr-1.5 text-emerald-300"></i>{activeClubs} active clubs</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80"><i className="ri-group-line mr-1.5 text-blue-300"></i>{totalMembers} members</span></div></div>
            ...
            ].map(([label, value, icon, colour]) => <div key={String(label)} className="flex min-w-0 items-center gap-2.5 rounded-xl border border-white/[0.09] bg-white/[0.08] p-3 backdrop-blur-sm sm:rounded-2xl sm:p-3.5"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/[0.08]"><i className={`${icon} ${colour}`}></i></span><div className="min-w-0"><p className="text-lg font-bold text-white sm:text-xl">{loading ? '–' : value}</p><p className="truncate text-[10px] text-white/60">{label}</p></div></div>)}</div>
          </div>
        </section>
...
function Loading() { return <div className="rounded-2xl border border-background-200 bg-white px-4 py-10 text-center text-sm text-foreground-400 sm:rounded-3xl sm:p-14"><i className="ri-loader-4-line mr-2 animate-spin text-primary-600"></i>Loading clubs from the database…</div>; }
function EmptyState({ message }: { message: string }) { return <div className="rounded-2xl border border-dashed border-foreground-300 bg-white px-4 py-10 text-center sm:rounded-3xl sm:px-6 sm:py-14"><span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-primary-50 text-primary-500"><i className="ri-inbox-line text-xl"></i></span><p className="mt-3 text-sm font-semibold text-foreground-700">{message}</p></div>; }
