  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-foreground-200/70 bg-white p-5 shadow-[0_5px_24px_rgba(28,10,55,0.05)] transition-all duration-300 hover:-translate-y-1 hover:border-primary-200 hover:shadow-[0_16px_38px_rgba(68,30,115,0.12)]">
      ...
      <div className="flex items-start gap-3 pt-1">
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-100 to-secondary-100 text-primary-700 ring-1 ring-primary-100"><i className="ri-community-line text-xl"></i></span>
        <div className="min-w-0 flex-1">
          <h3 className="truncate text-base font-bold text-foreground-900">{club.name}</h3>
          <p className="mt-1 flex items-center gap-1 text-xs text-foreground-400"><i className="ri-map-pin-2-line text-primary-500"></i>{club.location || 'Location not set'}</p>
        </div>
      ...
      {nextMeeting && (
        <div className="mt-4 flex items-center gap-3 rounded-2xl border border-primary-100 bg-gradient-to-r from-primary-50 to-secondary-50/60 p-3 text-xs">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white text-primary-600 shadow-sm"><i className="ri-calendar-check-line"></i></span>
...
function Metric({ icon, value, label, colour }: { icon: string; value: number; label: string; colour: string }) {
  return <div className="flex items-center gap-2"><span className={`flex h-8 w-8 items-center justify-center rounded-xl bg-white shadow-sm ${colour}`}><i className={icon}></i></span><div><p className="text-sm font-bold text-foreground-900">{value}</p><p className="text-[9px] text-foreground-400">{label}</p></div></div>;
}
...
function MeetingRow({ meeting, club }: { meeting: EngagementClubMeeting; club: EngagementClub }) {
  return (
    <div className="grid gap-3 px-4 py-4 transition hover:bg-primary-50/30 sm:grid-cols-[48px_minmax(200px,1.5fr)_minmax(150px,1fr)_minmax(180px,1fr)_110px] sm:items-center sm:px-5">
      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-50 text-primary-600"><i className="ri-calendar-event-line"></i></span>
      ...
      <div className="text-xs text-foreground-600"><i className="ri-map-pin-line mr-1.5 text-primary-500"></i>{meeting.venue || 'Venue TBC'}</div>
      <div className="text-xs text-foreground-600"><i className="ri-calendar-line mr-1.5 text-primary-500"></i>{meeting.date || 'Date TBC'}{meeting.time ? ` · ${meeting.time}` : ''}</div>
...
function EventCard({ event, booking, busy, onBook, onCancel }: { event: EngagementEvent; booking?: EventBooking; busy: boolean; onBook: () => void; onCancel: () => void }) {
  ...
  return (
    <article className="rounded-3xl border border-foreground-200/70 bg-white p-5 shadow-[0_5px_24px_rgba(28,10,55,0.05)] transition hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-secondary-100 text-secondary-700"><i className="ri-calendar-event-line text-lg"></i></span><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold capitalize ${statusClass}`}>{event.status}</span></div>
      ...
      <div className="mt-4 space-y-2.5 rounded-2xl bg-background-100/70 p-3 text-xs text-foreground-500"><p><i className="ri-calendar-line mr-2 text-primary-500"></i>{event.date} · {event.time}</p><p><i className="ri-map-pin-line mr-2 text-primary-500"></i>{event.location}</p><p><i className="ri-user-star-line mr-2 text-primary-500"></i>{event.organizer}</p><p><i className="ri-group-line mr-2 text-primary-500"></i>{event.attendees} attendees</p></div>
      {booking ? <div className="mt-4 flex items-center gap-2"><span className="flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-50 text-xs font-bold text-emerald-700 ring-1 ring-inset ring-emerald-200"><i className="ri-checkbox-circle-fill"></i>Place booked</span><button onClick={onCancel} disabled={busy} className="h-10 rounded-xl border border-foreground-200 px-3 text-[10px] font-semibold text-foreground-500 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600 disabled:opacity-50">{busy ? <i className="ri-loader-4-line animate-spin"></i> : 'Cancel'}</button></div> : <button onClick={onBook} disabled={busy || event.status === 'completed'} className="mt-4 h-10 w-full rounded-xl bg-primary-700 text-xs font-bold text-white shadow-md shadow-primary-700/20 transition hover:bg-primary-800 disabled:cursor-not-allowed disabled:bg-background-200 disabled:text-foreground-400 disabled:shadow-none">{busy ? <i className="ri-loader-4-line animate-spin"></i> : event.status === 'completed' ? 'Booking closed' : 'Book place'}</button>}
    </article>
  );
}
      ...
      <main className="w-full space-y-5 p-4 md:p-6">
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-6 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] md:p-7">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_480px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-secondary-100"><i className="ri-community-line text-secondary-300"></i>Learner community</span><h1 className="mt-3 text-2xl font-bold text-white md:text-3xl">Find your community at KBC</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/65">Explore active clubs, meet other learners and keep up with community meetings and events.</p><div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/75"><i className="ri-checkbox-circle-line mr-1.5 text-emerald-300"></i>{activeClubs} active clubs</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/75"><i className="ri-group-line mr-1.5 text-blue-300"></i>{totalMembers} members</span></div></div>
        ...
        <section className="sticky top-2 z-10 rounded-2xl border border-foreground-200/70 bg-white/95 p-2.5 shadow-[0_8px_30px_rgba(31,14,59,0.08)] backdrop-blur-xl">
          <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between"><div className="flex gap-1.5 overflow-x-auto">{tabs.map((tab) => <button key={tab.key} onClick={() => setActiveSection(tab.key)} className={`flex items-center gap-2 whitespace-nowrap rounded-xl px-4 py-2.5 text-xs font-semibold transition-all ${activeSection === tab.key ? 'bg-primary-700 text-white shadow-md shadow-primary-700/20' : 'text-foreground-500 hover:bg-primary-50 hover:text-primary-700'}`}><i className={tab.icon}></i>{tab.label}<span className={`rounded-full px-1.5 py-0.5 text-[9px] ${activeSection === tab.key ? 'bg-white/15 text-white' : 'bg-background-100 text-foreground-400'}`}>{counts[tab.key]}</span></button>)}</div>{activeSection === 'clubs' && <label className="relative block lg:w-72"><i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-foreground-400"></i><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by club, place or ambassador…" className="h-10 w-full rounded-xl border border-foreground-200 bg-background-50 pl-9 pr-3 text-xs outline-none transition focus:border-primary-400 focus:bg-white focus:ring-4 focus:ring-primary-100" /></label>}</div>
        </section>
...
function ContentHeader({ title, subtitle, count, children }: { title: string; subtitle: string; count: number; children: React.ReactNode }) {
  return <section><div className="mb-4 flex items-end justify-between"><div><h2 className="text-lg font-bold text-foreground-900">{title}</h2><p className="mt-1 text-xs text-foreground-500">{subtitle}</p></div><span className="rounded-full border border-background-200 bg-white px-3 py-1 text-[10px] font-semibold text-foreground-500">{count} {count === 1 ? 'record' : 'records'}</span></div>{children}</section>;
}

function Loading() { return <div className="rounded-3xl border border-background-200 bg-white p-14 text-center text-sm text-foreground-400"><i className="ri-loader-4-line mr-2 animate-spin text-primary-600"></i>Loading clubs from the database…</div>; }
function EmptyState({ message }: { message: string }) { return <div className="rounded-3xl border border-dashed border-foreground-300 bg-white px-6 py-14 text-center"><span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-primary-50 text-primary-500"><i className="ri-inbox-line text-xl"></i></span><p className="mt-3 text-sm font-semibold text-foreground-700">{message}</p></div>; }
