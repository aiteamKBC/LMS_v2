      ...
      <main className="w-full space-y-4 p-3 sm:p-4 md:space-y-5 md:p-6">
        <section className="learner-super-admin-hero relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-4 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] sm:rounded-3xl sm:p-6 md:p-7">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_480px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-secondary-100"><AppIcon className="ri-community-line text-secondary-300"></AppIcon>Learner community</span><h1 className="mt-3 text-[22px] font-bold leading-tight text-white sm:text-2xl md:text-3xl">Your community at KBC</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/70">Clubs are assigned by your coach — see who's in yours, keep up with meetings and book community events.</p><div className="mt-4 flex flex-wrap gap-2 sm:mt-5"><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80"><AppIcon className="ri-checkbox-circle-line mr-1.5 text-emerald-300"></AppIcon>{activeClubs} active club{activeClubs === 1 ? '' : 's'}</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80"><AppIcon className="ri-group-line mr-1.5 text-blue-300"></AppIcon>{totalMembers} members</span></div></div>
