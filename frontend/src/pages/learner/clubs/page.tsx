    ...
    <WorkspaceShell role="learner" roleLabel={learnerNav.label} navItems={learnerNav.items} workspaceLabel={learnerNav.workspaceLabel} pageTitle="Clubs" pageSubtitle="Connect, learn and grow with fellow apprentices" userName={learner?.name || 'Learner'} userRole={learner?.programme ? `${learner.programme} Apprentice` : 'Apprentice'}>
      <main className="page-container min-w-0 w-full space-y-3 p-3 md:space-y-4 md:p-6">
        <section className="learner-super-admin-hero relative overflow-hidden rounded-2xl p-4 text-primary-800 sm:rounded-3xl sm:p-6 md:p-6 workspace-page-hero">
          <div className="pointer-events-none absolute -right-20 -top-32 h-80 w-80 rounded-full bg-fuchsia-300/15 blur-3xl hidden"></div><div className="pointer-events-none absolute bottom-0 left-1/3 h-32 w-64 rounded-full bg-primary-300/10 blur-3xl hidden"></div>
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_480px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full border border-primary-200/60 bg-primary-100/60 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-primary-600"><AppIcon className="ri-community-line text-primary-600"></AppIcon>Learner community</span><h1 className="mt-3 text-[22px] font-bold leading-tight text-primary-800 sm:text-2xl md:text-3xl">Your community at KBC</h1><p className="mt-2 max-w-xl text-sm leading-6 text-foreground-500">Clubs are assigned by your coach — see who's in yours, keep up with meetings and book community events.</p><div className="mt-4 flex flex-wrap gap-2 sm:mt-5"><span className="rounded-full bg-primary-100/60 px-3 py-1.5 text-xs text-foreground-500"><AppIcon className="ri-checkbox-circle-line mr-1.5 text-emerald-300"></AppIcon>{activeClubs} active club{activeClubs === 1 ? '' : 's'}</span><span className="rounded-full bg-primary-100/60 px-3 py-1.5 text-xs text-foreground-500"><AppIcon className="ri-group-line mr-1.5 text-blue-300"></AppIcon>{totalMembers} members</span></div></div>
        ...
        {loading ? <Loading /> : activeSection === 'clubs' ? <ContentHeader title="Your clubs" subtitle="Clubs your coach has assigned you to" count={filteredClubs.length}>{filteredClubs.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filteredClubs.map((club) => <ClubCard key={club.id} club={club} />)}</div> : <EmptyState message={query ? 'No clubs match your search.' : "You haven't been assigned to a club yet — ask your coach."} />}</ContentHeader>
