    ...
    >
      <main className="w-full space-y-5 p-4 md:p-6">
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-6 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] md:p-7">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-secondary-100"><i className="ri-award-line text-amber-300"></i>Recognition & rewards</span><h1 className="mt-3 text-2xl font-bold text-white md:text-3xl">Turn your progress into rewards</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">See every point earned, celebrate recognition and claim available rewards.</p></div><div className="rounded-2xl border border-white/10 bg-white/10 px-5 py-3 backdrop-blur"><p className="text-[10px] uppercase tracking-wider text-white/50">Available balance</p><p className="mt-1 text-3xl font-bold text-white">{loading ? '–' : available.toLocaleString()} <span className="text-xs font-medium text-amber-300">pts</span></p></div></div>
          ...
          ].map(([label, value, icon, colour]) => <div key={String(label)} className="rounded-2xl border border-white/[0.08] bg-white/[0.07] p-3"><i className={`${icon} ${colour}`}></i><p className="mt-1 text-xl font-bold text-white">{loading ? '–' : Number(value).toLocaleString()}</p><p className="text-[10px] text-white/50">{label}</p></div>)}</div>
        </section>
        ...
        <section className="sticky top-2 z-10 rounded-2xl border border-foreground-200/70 bg-white/95 p-2.5 shadow-[0_8px_30px_rgba(31,14,59,0.08)] backdrop-blur-xl">
          <div className="flex gap-1.5 overflow-x-auto">{tabs.map((item) => { const count = item.key === 'shop' ? rewards.length : item.key === 'claims' ? claims.length : item.key === 'recognition' ? recognitions.length : grants.length; return <button key={item.key} onClick={() => setTab(item.key)} className={`flex items-center gap-2 whitespace-nowrap rounded-xl px-4 py-2.5 text-xs font-semibold transition ${tab === item.key ? 'bg-primary-700 text-white shadow-md shadow-primary-700/20' : 'text-foreground-500 hover:bg-primary-50 hover:text-primary-700'}`}><i className={item.icon}></i>{item.label}<span className={`rounded-full px-1.5 py-0.5 text-[9px] ${tab === item.key ? 'bg-white/15 text-white' : 'bg-background-100 text-foreground-400'}`}>{count}</span></button>; })}</div>
        </section>

        {loading ? <Loading /> : tab === 'shop' ? <section><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-bold text-foreground-900">Rewards shop</h2><p className="mt-1 text-xs text-foreground-500">Available items from Engagement.rewards</p></div><div className="flex gap-2"><select value={category} onChange={(event) => setCategory(event.target.value)} className="h-10 rounded-xl border border-foreground-200 bg-white px-3 text-xs outline-none"><option value="all">All categories</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><label className="relative"><i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-foreground-400"></i><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search rewards…" className="h-10 w-56 rounded-xl border border-foreground-200 pl-9 pr-3 text-sm outline-none focus:border-primary-400" /></label></div></div>{filteredRewards.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filteredRewards.map((reward) => <RewardCard key={reward.id} reward={reward} available={available} onClaim={setSelected} />)}</div> : <Empty icon="ri-gift-line" title="No rewards found" />}</section>
...
function Loading() { return <div className="rounded-2xl border border-background-200 bg-white p-14 text-center text-sm text-foreground-400"><i className="ri-loader-4-line mr-2 animate-spin text-primary-600"></i>Loading rewards from the database…</div>; }
function Empty({ icon, title }: { icon: string; title: string }) { return <div className="rounded-2xl border border-dashed border-foreground-300 bg-white px-6 py-14 text-center"><span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-primary-50 text-primary-500"><i className={`${icon} text-xl`}></i></span><p className="mt-3 text-sm font-semibold text-foreground-700">{title}</p></div>; }
