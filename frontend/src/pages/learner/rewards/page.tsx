...
function RewardCard({ reward, available, onClaim }: { reward: Reward; available: number; onClaim: (reward: Reward) => void }) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-foreground-200/70 bg-white shadow-[0_5px_24px_rgba(28,10,55,0.05)] transition-all duration-300 hover:-translate-y-1 hover:border-primary-200 hover:shadow-[0_16px_38px_rgba(68,30,115,0.12)]">
      ...
      {reward.image ? <div className="flex h-44 items-center justify-center border-b border-background-200 bg-gradient-to-br from-white to-background-100 p-5"><img src={reward.image} alt={reward.name} className="h-full w-full object-contain transition duration-300 group-hover:scale-[1.03]" /></div> : <div className="flex h-44 items-center justify-center border-b border-primary-100 bg-gradient-to-br from-primary-50 to-secondary-100 text-primary-500"><AppIcon className="ri-gift-2-line text-4xl"></AppIcon></div>}
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4 flex items-center justify-between rounded-2xl bg-background-100/70 px-4 py-3"><div className="flex items-center gap-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-amber-600 shadow-sm"><AppIcon className="ri-coins-line"></AppIcon></span><div><p className="text-lg font-bold leading-none text-primary-700">{reward.points.toLocaleString()}</p><p className="mt-1 text-[9px] uppercase tracking-wider text-foreground-400">Points</p></div></div><div className="text-right"><p className="text-xs font-semibold text-foreground-700">{remaining} left</p><p className="mt-1 text-[9px] capitalize text-foreground-400">{reward.deliveryType} delivery</p></div></div>
    ...
    >
      <main className="w-full space-y-4 p-3 sm:p-4 md:space-y-5 md:p-6">
        <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#17032d] via-[#33105e] to-[#6a2ca0] p-4 text-white shadow-[0_18px_50px_rgba(39,12,73,0.18)] sm:rounded-3xl sm:p-6 md:p-7">
          ...
          <div className="relative flex flex-col gap-4 sm:gap-5 lg:flex-row lg:items-end lg:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-secondary-100"><AppIcon className="ri-award-line text-amber-300"></AppIcon>Recognition & rewards</span><h1 className="mt-3 text-[22px] font-bold leading-tight text-white sm:text-2xl md:text-3xl">Turn your progress into rewards</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/75">See every point earned, celebrate recognition and claim available rewards.</p></div><div className="rounded-xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur sm:rounded-2xl sm:px-5"><p className="text-[10px] uppercase tracking-wider text-white/60">Available balance</p><p className="mt-1 text-2xl font-bold text-white sm:text-3xl">{loading ? '–' : available.toLocaleString()} <span className="text-xs font-medium text-amber-300">pts</span></p></div></div>
          ...
          ].map(([label, value, icon, colour]) => <div key={String(label)} className="min-w-0 rounded-xl border border-white/[0.08] bg-white/[0.07] p-2.5 sm:rounded-2xl sm:p-3"><AppIcon className={`${icon} ${colour}`}></AppIcon><p className="mt-1 text-lg font-bold text-white sm:text-xl">{loading ? '–' : Number(value).toLocaleString()}</p><p className="truncate text-[9px] text-white/60 sm:text-[10px]">{label}</p></div>)}</div>
        </section>
