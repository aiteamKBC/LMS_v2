...
export function moduleMeasures(module: TimelineModule, data: TrainingPlanDashboard): ModuleMeasure[] {
  const marked = [...new Map(module.sessions.filter(session => !['cancelled', 'deleted'].includes(session.status)
    && typeof session.attended === 'boolean').map(session => [session.id, session])).values()];
  const attended = marked.filter(session => session.attended).length;
  ...
  return [
    { label: 'Attendance', value: ratio(attended, marked.length), detail: marked.length ? `${attended} / ${marked.length} marked sessions attended` : 'Attendance not recorded yet' },
...
export function moduleProgress(module: TimelineModule, data: TrainingPlanDashboard) {
  const measures = moduleMeasures(module, data);
