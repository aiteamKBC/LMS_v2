...
export function StudentActivityPanel({ data, loading, error, onRetry, kind, learnerId, real = null, onProgress }: {
  ...
  const total = subjects.reduce((sum, subject) => sum + subject.activities.length, 0);
  const done = subjects.reduce((sum, subject) => sum + subject.activities.filter((entry) => entry.completed).length, 0);
  if (loading && !data && !subjects.length) return <div role="status" className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map((key) => <div key={key} className="h-72 animate-pulse rounded-2xl bg-foreground-100" />)}<span className="sr-only">Loading subjects</span></div>;
  ...
  return <section className="space-y-5" aria-label="Your subjects">
    ...
    {!active ? <>
      ...
      {visible.length ? <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{visible.map((subject) => <article key={subject.id} className="relative overflow-hidden rounded-2xl border border-foreground-200 bg-white shadow-sm transition-shadow hover:shadow-md">
        {imageControl(subject)}<button onClick={() => setSelected(subject.id)} className="block h-full w-full text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary-500">
          <Cover title={subject.title} url={covers[subject.id]} /><div className="space-y-4 p-5"><h3 className="min-h-12 text-base font-bold leading-6 text-foreground-900">{subject.title}</h3><Progress done={subject.activities.filter((entry) => entry.completed).length} total={subject.activities.length} /><span className="flex items-center justify-between border-t border-foreground-100 pt-3 text-xs font-semibold text-primary-700">Open subject<ChevronRight size={16} /></span></div>
