...
import { getWeekData, CURRENT_GLOBAL_WEEK, TOTAL_WEEKS } from '@/mocks/training-plan';
...
const statusStyle: Record<string, { bg: string; text: string; dot: string; border: string }> = {
  'Not Started': { bg: 'bg-background-50', text: 'text-foreground-500', dot: 'bg-foreground-300', border: 'border-background-200/50' },
  ...
  'overdue': { bg: 'bg-red-50/40', text: 'text-red-700', dot: 'bg-red-500', border: 'border-red-200/60' },
  'upcoming': { bg: 'bg-background-50', text: 'text-foreground-500', dot: 'bg-foreground-300', border: 'border-background-200/50' },
};
...
export default function WeekDetailPage() {
  ...
  const weekData = useMemo(() => getWeekData(weekNum), [weekNum]);

  const [components, setComponents] = useState(weekData.components);
  const [selectedComp, setSelectedComp] = useState<typeof components[number] | null>(null);
  const [videoComp, setVideoComp] = useState<typeof components[number] | null>(null);
  const [quizComp, setQuizComp] = useState<typeof components[number] | null>(null);
  const [readingComp, setReadingComp] = useState<typeof components[number] | null>(null);
  const [podcastComp, setPodcastComp] = useState<typeof components[number] | null>(null);
  const [evidenceComp, setEvidenceComp] = useState<typeof components[number] | null>(null);

  // Update components when week changes
  useEffect(() => {
    setComponents(weekData.components);
  }, [weekData.components]);

  const getQuizData = (_compId: string) => QUIZ_1_DATA;
  const getReadingData = (_compId: string) => READING_1_DATA;
  const getPodcastData = (_compId: string) => PODCAST_1_DATA;
  ...
  const handleVideoComplete = () => {
    ...
    setComponents(prev =>
      prev.map(c =>
        c.id === videoComp.id
          ? { ...c, status: 'In Progress' as const, primaryAction: 'Continue Learning', primaryIcon: 'ri-play-circle-line' }
          : c
      )
    );
    setVideoComp(null);
  ...
  const handleReadingPodcastComplete = () => {
    if (readingComp) {
      setComponents(prev =>
        prev.map(c =>
          c.id === readingComp.id
  ...
  const goToPrevWeek = () => {
    if (weekNum > 1) navigate(`/learner/week/${weekNum - 1}`);
  };

  const goToNextWeek = () => {
    if (weekNum < TOTAL_WEEKS) navigate(`/learner/week/${weekNum + 1}`);
  };

  const isCurrentWeek = weekNum === CURRENT_GLOBAL_WEEK;
  const isPastWeek = weekNum < CURRENT_GLOBAL_WEEK;
  const isFutureWeek = weekNum > CURRENT_GLOBAL_WEEK;

  const weekStatusBadge = isCurrentWeek ? 'Current Week' : isPastWeek ? 'Past Week' : 'Upcoming Week';
  const weekStatusColor = isCurrentWeek ? 'bg-accent-400/20 text-accent-300 border-accent-400/25' : isPastWeek ? 'bg-emerald-400/20 text-emerald-300 border-emerald-400/25' : 'bg-foreground-400/20 text-foreground-300 border-foreground-400/25';
  ...
  return (
    <WorkspaceShell
      ...
      pageTitle={`Week ${weekNum}`}
      pageSubtitle={`${weekData.modulePeriod.label} · ${weekData.dateRange.start} – ${weekData.dateRange.end}`}
      userName={p.fullName}
    ...
    >
      <div className="p-3 md:p-6 space-y-5 md:space-y-6">
        ...
        {/* ═══════════════════════════════════════════════════
            WEEK NAVIGATION BAR
            ═══════════════════════════════════════════════════ */}
        <div className="flex items-center justify-between bg-background-50 border border-foreground-200/50 rounded-xl px-4 py-3">
          <button
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${weekStatusColor}`}>
              {weekStatusBadge}
            </span>
            {isCurrentWeek && (
              <Link
        ...
        {/* ═══════════════════════════════════════════════════
            SECTION 4 — WEEKLY LEARNING COMPONENTS
            ═══════════════════════════════════════════════════ */}
        ...
        <SectionReveal delay={180}>
          <section className="bg-background-50 rounded-xl border border-foreground-300/50 p-5 md:p-6">
            ...
            <div className="mt-5 border-t border-background-200/50 pt-4">
              <h3 className="text-sm font-semibold text-foreground-700 mb-3">Off-the-job hours builder</h3>
              <p className="text-xs text-foreground-400 mb-3">Hours build automatically as you complete weekly components. Week {weekNum} of {TOTAL_WEEKS}.</p>
              <div className="overflow-x-auto rounded-xl border border-foreground-300/50">
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="bg-background-100/80 border-t border-background-200/50 font-medium">
        ...
        {/* ═══════════════════════════════════════════════════
            SECTION 7 — DEADLINES & KEY DATES
            ═══════════════════════════════════════════════════ */}
        <SectionReveal delay={260}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 bg-background-50 rounded-xl border border-foreground-300/50 p-5 md:p-6">
              <h2 className="text-base font-heading font-semibold text-foreground-900 mb-4">Deadlines &amp; Key Dates</h2>
              <div className="space-y-2">
                {weekData.deadlines.map((dl, i) => (
        ...
        <SectionReveal delay={340}>
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <GuidanceCard role="Tutor" name={p.tutor.name} initials={p.tutor.avatar} color="primary" guidance={weekData.tutorGuidance} />
              <GuidanceCard role="Coach" name={p.coach.name} initials={p.coach.avatar} color="accent" guidance={weekData.coachGuidance} />
            </div>
          </section>
        </SectionReveal>
      ...
      {evidenceComp && (
        <EvidenceLoggingModal
          ...
          weekNumber={weekNum}
          moduleName={weekData.modulePeriod.label}
          ...
          referralReason={evidenceComp.referralReason || null}
          requiredActions={evidenceComp.requiredActions || null}
        />
      )}
    </WorkspaceShell>
  );
...
function CompactComponentCard({
  ...
  component: ReturnType<typeof getWeekData>['components'][number];
  onClick: () => void;
}) {
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function ComponentDetailPanel({ component: c, onCtaClick }: { component: ReturnType<typeof getWeekData>['components'][number]; onCtaClick: () => void }) {
  const ts = typeStyle[c.type] || typeStyle['Evidence'];
  const ss = statusStyle[c.status] || statusStyle['Not Started'];
  const isCompleted = c.status === 'Completed';
  const isReferred = c.status === 'Referred';
  ...
  const ctaConfig: Record<string, { bg: string; hoverBg: string; border?: string; textColor?: string }> = {
    ...
    'Completed': { bg: 'bg-background-50', hoverBg: 'hover:bg-background-100', border: 'border border-background-200', textColor: 'text-foreground-600' },
  };
...
function KSBCard({ ksb }: { ksb: ReturnType<typeof getWeekData>['ksbs'][number] }) {
  const typeStyle2 = {
...
function DeadlineRow({ deadline }: { deadline: ReturnType<typeof getWeekData>['deadlines'][number] }) {
  const p = {
    completed: { dot: 'bg-emerald-500', text: 'text-foreground-400 line-through', badge: 'bg-emerald-50 text-emerald-700', label: 'Done', icon: 'ri-check-line' },
    today: { dot: 'bg-accent-500', text: 'text-foreground-900 font-semibold', badge: 'bg-accent-100 text-accent-700', label: 'Today', icon: 'ri-flashlight-line' },
    'due-this-week': { dot: 'bg-red-500', text: 'text-foreground-900', badge: 'bg-red-50 text-red-600', label: 'Due This Week', icon: 'ri-calendar-line' },
    upcoming: { dot: 'bg-foreground-300', text: 'text-foreground-500', badge: 'bg-background-100 text-foreground-500', label: 'Upcoming', icon: 'ri-calendar-event-line' },
  }[deadline.priority];

  return (
    <div className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-background-100 transition-smooth">
      <div className="flex items-center gap-3 min-w-0">
        <span className={`w-2 h-2 rounded-full ${p.dot} shrink-0`}></span>
        <span className={`text-sm ${p.text} truncate`}>{deadline.title}</span>
      </div>
...
function ResourceCard({ resource }: { resource: ReturnType<typeof getWeekData>['resources'][number] }) {
  return (
    <a
      href={resource.href}
...
function getCtaLabelAndIcon(type: string, status: string) {
  const icons: Record<string, Record<string, { label: string; icon: string }>> = {
    ...
    'overdue': { default: { label: 'Complete Now', icon: 'ri-flashlight-line' } },
  };
...
function getComponentStatusBadge(status: string) {
  const map: Record<string, { label: string; bg: string; text: string; dot: string }> = {
    ...
    'upcoming': { label: 'Upcoming', bg: 'bg-background-100', text: 'text-foreground-500', dot: 'bg-foreground-300' },
  };
