...
interface InitialAssessmentHeaderProps {
  record: InitialAssessmentRecord;
}

export function InitialAssessmentHeader({ record }: InitialAssessmentHeaderProps) {
  const statusConfig = getOverallStatusConfig(record.overallStatus);
  const readinessConfig = getReadinessConfig(record.readiness.band);
    ...
    <div className="bg-background-50 rounded-xl border border-background-200/50 p-5">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          ...
          <span className={`text-[12px] font-semibold px-3 py-1.5 rounded-full whitespace-nowrap ${readinessConfig.bg} ${readinessConfig.text}`}>
            {record.readiness.percentage}% Readiness
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <QuickStat label="English Level" value={record.englishLevel} icon="ri-english-input" />
        <QuickStat label="Maths Level" value={record.mathsLevel} icon="ri-calculator-line" />
        <QuickStat label="Digital Skills" value={record.digitalSkillsLevel} icon="ri-computer-line" />
        <QuickStat label="Learning Style" value={record.learningStyle.primaryStyle} icon="ri-lightbulb-line" />
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-background-200/50">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-1.5 text-[12px] text-primary-600 hover:text-primary-700 font-medium whitespace-nowrap cursor-pointer">
            <AppIcon className="ri-history-line"></AppIcon> Audit Trail
          </button>
        </div>
      </div>
...
function getOverallStatusConfig(status: string): { bg: string; text: string } {
  const map: Record<string, { bg: string; text: string }> = {
    ...
    'Awaiting BKSB': { bg: 'bg-primary-50', text: 'text-primary-700' },
    'Awaiting Diagnostics': { bg: 'bg-primary-50', text: 'text-primary-700' },
    'Assessed': { bg: 'bg-primary-50', text: 'text-primary-700' },
    'Below Required Level': { bg: 'bg-red-50', text: 'text-red-700' },
    'Requires LSP': { bg: 'bg-amber-50', text: 'text-amber-700' },
    'Ready for Programme': { bg: 'bg-emerald-50', text: 'text-emerald-700' },
    'Escalated': { bg: 'bg-secondary-50', text: 'text-secondary-700' },
  };
  return map[status] || { bg: 'bg-background-100', text: 'text-foreground-500' };
}

function getReadinessConfig(band: string): { bg: string; text: string } {
  const map: Record<string, { bg: string; text: string }> = {
    'Ready': { bg: 'bg-emerald-50', text: 'text-emerald-700' },
    'Ready with Support': { bg: 'bg-amber-50', text: 'text-amber-700' },
    'Requires Development': { bg: 'bg-amber-50', text: 'text-amber-700' },
    'Not Ready': { bg: 'bg-red-50', text: 'text-red-700' },
  };
  return map[band] || { bg: 'bg-background-100', text: 'text-foreground-500' };
}
