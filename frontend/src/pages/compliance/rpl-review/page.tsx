...
import { RPL_STATS } from '@/mocks/rpl-review';
import { RPL_REVIEW_LEARNERS } from '@/mocks/rpl-review-data';
import type { RPLRecord } from '@/mocks/rpl-review';
import { RPLHeader } from './components/RPLHeader';
import { KSBMappingTable } from './components/KSBMappingTable';
...
const TABS = [
  ...
  { id: 'ksb-mapping', label: 'KSB Mapping' },
...
export default function RPLReviewPage() {
  const [selectedRecord, setSelectedRecord] = useState<RPLRecord>(RPL_REVIEW_LEARNERS[0]);
  const [activeTab, setActiveTab] = useState<TabId>('overview');
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [riskFilter, setRiskFilter] = useState<string>('all');

  const filteredRecords = RPL_REVIEW_LEARNERS.filter(r => {
  ...
  const activeStatuses = [...new Set(RPL_REVIEW_LEARNERS.map(r => r.overallStatus))];
  ...
  return (
    <WorkspaceShell
      ...
      pageTitle="RPL / Skills Scan"
      pageSubtitle="Recognition of Prior Learning — KSB mapping, experience-to-standards crosswalk, and reduction-in-duration calculation"
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          ...
          <div className="lg:col-span-3 space-y-5">
            <RPLHeader record={selectedRecord} />
            ...
            <div className="max-w-3xl space-y-5">
              {activeTab === 'overview' && (
                <>
                  <KSBMappingTable ksbCategories={selectedRecord.ksbCategories} rplPercentage={selectedRecord.rplDecision.rplPercentage} />
                  <DurationCalculator reduction={selectedRecord.durationReduction} crosswalk={selectedRecord.experienceCrosswalk} />
                  {selectedRecord.rplDecision.summary && (
                    <section className="bg-background-50 rounded-xl border border-background-200/50 p-5">
                      <h3 className="text-[15px] font-heading font-semibold text-foreground-900 mb-3">RPL Decision Summary</h3>
                      <p className="text-[13px] text-foreground-600 leading-relaxed">{selectedRecord.rplDecision.summary}</p>
                      {selectedRecord.rplDecision.decidedBy && (
                        <p className="text-[11px] text-foreground-400 mt-3">
                          Decided by <span className="font-medium text-foreground-600">{selectedRecord.rplDecision.decidedBy}</span> on {formatDate(selectedRecord.rplDecision.decidedAt)}
                        </p>
                      )}
                    </section>
                  )}
...
function StatsBanner() {
  const stats = [
    { label: 'Total In RPL', value: RPL_STATS.totalInRPL, icon: 'ri-file-search-line', color: 'primary' as const },
    { label: 'RPL Approved', value: RPL_STATS.rplApproved, icon: 'ri-check-line', color: 'accent' as const },
    { label: 'In Progress', value: RPL_STATS.rplInProgress, icon: 'ri-time-line', color: 'secondary' as const },
    { label: 'RPL Rejected', value: RPL_STATS.rplRejected, icon: 'ri-close-line', color: 'secondary' as const },
  ];
...
function RecordsList({
  ...
  records: RPLRecord[]; selectedId: string; onSelect: (r: RPLRecord) => void;
  ...
  return (
    <div className="bg-background-50 rounded-xl border border-background-200/50 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-heading font-semibold text-foreground-900">RPL Records</h3>
      ...
      <div className="space-y-2 max-h-[calc(100vh-420px)] overflow-y-auto pr-1">
        {records.map(r => {
          const reductionText = r.durationReduction.rplReduction > 0 ? `-${r.durationReduction.rplReduction}m` : 'No reduction';
          return (
            <button key={r.id} onClick={() => onSelect(r)}
              className={`w-full text-left px-3 py-3 rounded-lg border transition-smooth cursor-pointer ${
                selectedId === r.id ? 'border-primary-300/60 bg-primary-50/40 shadow-sm' : 'border-background-200/40 bg-background-50 hover:border-background-300/60 hover:bg-background-50'
              }`}
            >
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                {r.rplDecision.rplPercentage > 0 && (
                  <span className="text-[10px] text-foreground-400">{r.rplDecision.rplPercentage}% RPL &middot; {reductionText}</span>
                )}
              </div>
            </button>
          );
...
function getBadgeConfig(status: string): { bg: string; text: string } {
  if (/approved|ready/i.test(status) && !/not/i.test(status)) return { bg: 'bg-emerald-50', text: 'text-emerald-600' };
  if (/in progress|evidence|applied/i.test(status)) return { bg: 'bg-primary-50', text: 'text-primary-600' };
  if (/rejected|ineligible/i.test(status)) return { bg: 'bg-red-50', text: 'text-red-600' };
  if (/not started/i.test(status)) return { bg: 'bg-background-200', text: 'text-foreground-400' };
