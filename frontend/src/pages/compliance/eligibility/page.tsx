...
import { ELIGIBILITY_STATS } from '@/mocks/eligibility-review';
import { ELIGIBILITY_REVIEW_LEARNERS } from '@/mocks/eligibility-review-data';
...
import { ResidencyTests } from './components/ResidencyTests';
import { FundingPanel } from './components/FundingPanel';
import { PriorAttainmentPanel } from './components/PriorAttainmentPanel';
import { EligibilityOutcome } from './components/EligibilityOutcome';
...
export default function EligibilityReviewPage() {
  const [selectedRecord, setSelectedRecord] = useState<EligibilityReviewRecord>(ELIGIBILITY_REVIEW_LEARNERS[0]);
  const [activeTab, setActiveTab] = useState<TabId>('overview');
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [riskFilter, setRiskFilter] = useState<string>('all');

  const filteredRecords = ELIGIBILITY_REVIEW_LEARNERS.filter(r => {
  ...
  return (
    <WorkspaceShell
      ...
      pageTitle="Eligibility Review"
      pageSubtitle="3-residency-test validation, funding eligibility, prior attainment assessment — confirm learner meets all ESFA funding rules"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            <div className="max-w-3xl space-y-5">
              {activeTab === 'overview' && (
                <>
                  <ResidencyTests tests={selectedRecord.residencyTests} />
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <FundingPanel checks={selectedRecord.fundingChecks} />
                    <PriorAttainmentPanel records={selectedRecord.priorAttainment} />
                  </div>
                  <EligibilityOutcome record={selectedRecord} />
                </>
              )}
              {activeTab === 'residency' && (
                <ResidencyTests tests={selectedRecord.residencyTests} />
...
function StatsBanner() {
  const stats = [
    { label: 'Total In Review', value: ELIGIBILITY_STATS.totalInReview, icon: 'ri-user-line', color: 'primary' as const },
    { label: 'Eligible', value: ELIGIBILITY_STATS.eligible, icon: 'ri-check-line', color: 'accent' as const },
    { label: 'Not Eligible', value: ELIGIBILITY_STATS.notEligible, icon: 'ri-close-line', color: 'secondary' as const },
    { label: 'Evidence Required', value: ELIGIBILITY_STATS.evidenceRequired, icon: 'ri-error-warning-line', color: 'secondary' as const },
  ];
...
function RecordsList({
  ...
  records: EligibilityReviewRecord[];
  selectedId: string;
  onSelect: (r: EligibilityReviewRecord) => void;
  searchQuery: string;
  ...
  return (
    <div className="bg-background-50 rounded-xl border border-background-200/50 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-heading font-semibold text-foreground-900">Eligibility Records</h3>
      ...
      <div className="space-y-2 max-h-[calc(100vh-420px)] overflow-y-auto pr-1">
        {records.map(r => {
          const residencyPassed = r.residencyTests.filter(t => t.status === 'pass').length;
          const decisionConfig = r.eligibilityOutcome.decision === 'eligible' ? 'Eligible'
            : r.eligibilityOutcome.decision === 'not-eligible' ? 'Not Eligible'
            : r.eligibilityOutcome.decision === 'conditionally-eligible' ? 'Conditional'
            : 'Pending';
          return (
            <button
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-[10px] text-foreground-400">{residencyPassed}/3 residency &middot; {decisionConfig}</span>
              </div>
            </button>
          );
        })}
...
function getBadgeConfig(status: string): { bg: string; text: string } {
  if (/submitted|under.*review/i.test(status)) return { bg: 'bg-primary-50', text: 'text-primary-600' };
  if (/eligible|ready/i.test(status) && !/not/i.test(status) && !/conditional/i.test(status)) return { bg: 'bg-emerald-50', text: 'text-emerald-600' };
  if (/conditional/i.test(status)) return { bg: 'bg-amber-50', text: 'text-amber-600' };
  if (/not eligible|rejected/i.test(status)) return { bg: 'bg-red-50', text: 'text-red-600' };
  if (/evidence.*required/i.test(status)) return { bg: 'bg-amber-50', text: 'text-amber-600' };
...
function ActionHistory({ actions, notes }: { actions: { action: string; by: string; timestamp: string; detail: string }[]; notes: { author: string; text: string; timestamp: string; visibility: string }[] }) {
  const allItems = [
