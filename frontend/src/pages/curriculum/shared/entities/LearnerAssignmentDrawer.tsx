...
import {
  assignCurriculumLearners, fetchLearnerAssignments,
...
function AssignmentForm({ target, onClose, onAssigned }: {
  ...
  const toggle = (learner: AssignmentLearner) => setSelected(previous => {
    const next = new Set(previous);
    if (next.has(learner.id)) next.delete(learner.id); else next.add(learner.id);
    return next;
  });
  const submit = async () => {
    if (!data || loading || !selected.size || savingRef.current) return;
    savingRef.current = true;
    setSaving(true);
    setError(null);
    try {
      const result = await assignCurriculumLearners(target, [...selected]);
      onAssigned(result);
      onClose();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Unable to assign learners. Please try again.');
    } finally {
  ...
  return (
    <EntityDrawer open title={`Assign learners to ${target.scope}`} subtitle={target.name}
      ...
      saving={saving} error={error} submitDisabled={loading || !data || !selected.size}
      submitLabel={saving ? 'Assigning…' : `Assign ${selected.size || ''} learner${selected.size === 1 ? '' : 's'}`}>
      <div role="note" className="rounded-xl border border-primary-200 bg-primary-50 p-3 text-[12px] leading-5 text-primary-900">
        {target.scope === 'cohort'
          ...
          : 'Selected learners will receive this module only. Their programme, cohort and other module assignments stay as they are.'}
      </div>
      <fieldset disabled={loading || saving} className="space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-[12px]">
          <span aria-live="polite">{visible.length} learners · {selected.size} selected{selected.size > visibleSelected ? ` (${selected.size - visibleSelected} outside these filters)` : ''}</span>
          <div className="flex gap-3">
            <button type="button" onClick={() => setSelected(new Set())} disabled={!selected.size} className="font-bold text-primary-700 disabled:opacity-40">Clear selection</button>
          ...
          : <div className="overflow-hidden rounded-xl border border-foreground-200">
            ...
            <div className="max-h-[42vh] overflow-auto">
              {visible.length === 0 ? <p className="p-8 text-center text-[12px] text-foreground-500">No learners match these filters.</p>
                : visible.map(learner => (
                  <label key={learner.id} className={`flex items-start gap-3 border-b border-foreground-100 p-4 last:border-0 ${learner.assigned ? 'bg-background-100/70' : 'hover:bg-primary-50/40'}`}>
                    <input className="mt-1" type="checkbox" aria-label={`Select ${learner.name}`} checked={learner.assigned || selected.has(learner.id)} disabled={saving || learner.assigned} onChange={() => toggle(learner)} />
                    ...
                    {learner.assigned && <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700"><AppIcon className="ri-check-line mr-1" />Already assigned</span>}
                  </label>
                ))}
            </div>
          </div>}
