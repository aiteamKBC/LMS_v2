...
const cards = (counts: CaseloadCounts): Array<{ label: string; value: number; note: string; icon: string; tone: string; filter: StatusFilter }> => [
  ...
  { label: 'On Track', value: counts.onTrack, note: 'OTJH progress is on track', icon: 'ri-checkbox-circle-fill', tone: 'positive', filter: 'on-track' },
  { label: 'Need Attention', value: counts.attention + counts.upcoming, note: 'OTJH progress needs review', icon: 'ri-error-warning-fill', tone: 'warning', filter: 'need-attention' },
  { label: 'At Risk', value: counts.critical, note: 'OTJH progress requires action', icon: 'ri-alert-fill', tone: 'critical', filter: 'at-risk' },
];
...
export function CaseloadSummary({ counts, value, onChange }: { counts: CaseloadCounts; value: StatusFilter; onChange: (value: StatusFilter) => void }) {
  return <section className={styles.summary} aria-label="OTJH caseload summary">
