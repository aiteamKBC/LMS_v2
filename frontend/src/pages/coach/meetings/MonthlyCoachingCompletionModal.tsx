        ...
        <header className="shrink-0 border-b border-white/10 bg-gradient-to-r from-[#10021f] via-primary-950 to-[#35105e] px-5 py-5 text-white sm:px-7">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-white/60">Complete the guided MCM record before closing the meeting.</p>
              </div>
            </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-secondary-300 transition-all" style={{ width: `${percent}%` }} />
            </div>
              ...
              <section key={section.id} className={`overflow-hidden rounded-2xl border bg-background-50 transition-all ${open ? 'border-primary-300 shadow-sm' : 'border-background-200'}`}>
                <button type="button" onClick={() => setOpenSection(open ? '' : section.id)} className="flex w-full items-center gap-3 p-4 text-left sm:px-5">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-[9px] font-bold uppercase tracking-[0.12em] text-foreground-400">MCM section {sectionIndex + 1} of {MONTHLY_COACHING_SECTIONS.length}</span>
                ...
                {open && (
                  <div className="space-y-3 border-t border-primary-100 bg-white p-4 sm:p-5">
                    {section.questions.map((question, questionIndex) => (
                      question.type === 'statement' ? (
                        <div key={question.id} className="rounded-xl border border-background-200 bg-background-100/70 px-4 py-3">
                          <div className="flex items-start gap-3">
                            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-100 text-[9px] font-bold text-primary-700">{questionIndex + 1}</span>
                            <div>
                              <p className="text-xs font-bold text-foreground-800">{question.label}</p>
                              {question.helpText && <p className="mt-1 text-[10px] leading-4 text-foreground-400">{question.helpText}</p>}
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div key={question.id} className="rounded-2xl border border-background-200 bg-background-50 p-4 transition focus-within:border-primary-300 focus-within:shadow-sm">
                          ...
                          {question.helpText && <p className="mb-2 text-[10px] leading-4 text-foreground-400">{question.helpText}</p>}

                          {question.type === 'text' && (
                            <textarea value={responses[question.id] || ''} onChange={(e) => update(question.id, e.target.value)} rows={3} maxLength={4000} placeholder={question.placeholder} className="w-full resize-y rounded-xl border border-background-300 bg-white px-3.5 py-3 text-sm text-foreground-800 outline-none transition placeholder:text-foreground-300 focus:border-primary-400 focus:ring-2 focus:ring-primary-200" />
                          )}
                          ...
                          {question.type === 'agreement' && (
                            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                              {['Agree', 'Neutral', 'Disagree', 'Not discussed'].map((option) => (
                                <button key={option} type="button" onClick={() => update(question.id, option)} className={`min-h-10 rounded-xl border px-3 py-2 text-[11px] font-bold transition ${responses[question.id] === option ? 'border-primary-600 bg-primary-600 text-white shadow-sm' : 'border-background-300 bg-white text-foreground-600 hover:border-primary-300 hover:bg-primary-50'}`}>
                                  {option}
                                </button>
                              ))}
                            </div>
                          ...
                          {question.type === 'date' && (
                            <input type="date" value={responses[question.id] || ''} onChange={(e) => update(question.id, e.target.value)} className="h-11 w-full max-w-sm rounded-xl border border-background-300 bg-white px-3.5 text-sm font-semibold text-foreground-800 outline-none transition focus:border-primary-400 focus:ring-2 focus:ring-primary-200" />
                          )}
                          {question.type === 'rag' && (
