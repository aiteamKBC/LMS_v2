      ...
      <div className="relative bg-background-50 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="relative p-6 border-b border-foreground-200/60 flex items-center justify-between shrink-0" style={{ background: 'linear-gradient(135deg, oklch(var(--primary-900)) 0%, oklch(var(--primary-800)) 100%)' }}>
          <div className="flex items-center gap-3">
            <span className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center"><AppIcon className="ri-file-chart-line text-white text-lg"></AppIcon></span>
            <div>
          ...
          {phase === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="bg-background-100 rounded-xl p-4 flex flex-wrap items-center gap-4 text-[11px] text-foreground-600">
                <span className="flex items-center gap-1"><AppIcon className="ri-user-line text-primary-500"></AppIcon> Coach: <strong className="text-foreground-900">{report.coach}</strong></span>
                <span className="flex items-center gap-1"><AppIcon className="ri-calendar-line text-primary-500"></AppIcon> Period: <strong className="text-foreground-900">{report.period}</strong></span>
                <span className="flex items-center gap-1"><AppIcon className="ri-time-line text-primary-500"></AppIcon> Generated: <strong className="text-foreground-900">{report.generatedAt}</strong></span>
              </div>
                        ...
                        <div key={mi} className="bg-background-100 rounded-lg p-3 space-y-1">
                          ...
                          {m.change && (
                            <p className={`text-[10px] font-medium ${m.trend === 'up' ? 'text-green-600' : m.trend === 'down' ? 'text-red-500' : 'text-foreground-500'}`}>
                              <AppIcon className={`mr-0.5 ${m.trend === 'up' ? 'ri-arrow-up-line' : m.trend === 'down' ? 'ri-arrow-down-line' : 'ri-subtract-line'}`}></AppIcon>
                              {m.change}
                            </p>
                          )}
                        </div>
