...
interface ProgressReviewCompletionModalProps {
...
export default function ProgressReviewCompletionModal({
  event,
  busy,
  error,
  onClose,
  onSubmit,
}: ProgressReviewCompletionModalProps) {
  ...
  return (
    <ModalShell busy={busy} onClose={onClose}>
      <ModalHeader
        eyebrow="Submit progress review"
        icon="ri-file-edit-line"
        title={`${event.learner || 'Learner'} · ${eventPeriodLabel(event)}`}
        subtitle="Complete the review record, then send it for the line manager signature."
        busy={busy}
        onClose={onClose}
        progressPercent={completionPercent}
        progressLabel={`${answeredCount}/${REQUIRED_PROGRESS_REVIEW_RESPONSE_IDS.length} answered`}
      />
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-background-100 p-4 sm:p-6">
        ...
        {PROGRESS_REVIEW_SECTIONS.map((section, sectionIndex) => {
          ...
          return (
            <section key={section.id} className={cn('overflow-hidden rounded-2xl border bg-background-50 transition-all', isOpen ? 'border-primary-300 shadow-sm' : 'border-background-200')}>
              <button type="button" onClick={() => setOpenSection(isOpen ? '' : section.id)} className="flex w-full items-center gap-3 p-4 text-left sm:px-5">
                <span className={cn('relative flex h-10 w-10 shrink-0 items-center justify-center rounded-lg', isOpen ? 'bg-primary-600 text-white' : 'bg-primary-50 text-primary-700')}>
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[12px] font-bold uppercase tracking-[0.12em] text-foreground-400">Review section {sectionIndex + 1} of {PROGRESS_REVIEW_SECTIONS.length}</span>
                ...
                {sectionComplete ? <AppIcon className="ri-checkbox-circle-fill text-lg text-emerald-500"></AppIcon> : null}
                <span className={cn('flex h-8 w-8 items-center justify-center rounded-full bg-background-100 text-foreground-500 transition-transform', isOpen && 'rotate-180')}>
                  ...
                  {visibleQuestions.map((question, questionIndex) => (
                    <div key={question.id} className="rounded-2xl border border-background-200 bg-background-50 p-4 transition focus-within:border-primary-300 focus-within:shadow-sm">
                      <label className="mb-2 block text-xs font-bold text-foreground-800">
                        ...
                        {question.label}
                        {(question.required !== false || question.showWhen) ? <span className="ml-1 text-red-500">*</span> : null}
                      </label>
                      {question.helpText ? <p className="mb-2 text-[12px] text-foreground-400">{question.helpText}</p> : null}
                      {question.type === 'text' ? (
                        <textarea
                          value={responses[question.id] || ''}
                          onChange={(event_) => updateResponse(question.id, event_.target.value)}
                          rows={3}
                          maxLength={4000}
                          placeholder={question.placeholder}
                          className="w-full resize-y rounded-lg border border-background-300 bg-white px-3.5 py-3 text-sm text-foreground-800 outline-none transition placeholder:text-foreground-300 focus:border-primary-400 focus:ring-2 focus:ring-primary-200"
                        />
                      ) : null}
                        ...
                        <div className="grid max-w-sm grid-cols-2 gap-2">
                          {[
                            ...
                            <button
                              ...
                              className={cn(
                                'flex h-11 items-center justify-center gap-2 rounded-lg border text-xs font-bold transition',
                        ...
                        <div>
                          <div className="grid grid-cols-5 gap-1.5 sm:grid-cols-10">
                            {Array.from({ length: 10 }, (_, index) => String(index + 1)).map((rating) => (
                              <button key={rating} type="button" onClick={() => updateResponse(question.id, rating)} className={cn('flex h-10 min-w-0 items-center justify-center rounded-lg border text-xs font-bold transition', responses[question.id] === rating ? 'border-primary-600 bg-primary-600 text-white shadow-sm' : 'border-background-300 bg-white text-foreground-600 hover:border-primary-300 hover:bg-primary-50')}>
                                {rating}
                              </button>
                            ))}
                          </div>
                          <div className="mt-2 flex justify-between text-[12px] font-medium text-foreground-400">
                      ...
                      {question.type === 'select' ? (
                        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                          {(question.options || []).map((option) => (
                            ...
                            >
                              <AppIcon className={cn('mr-2', responses[question.id] === option ? 'ri-checkbox-circle-fill' : 'ri-checkbox-blank-circle-line')}></AppIcon>
                              {option}
                            </button>
                          ))}
                        </div>
                      ...
                      {question.type === 'rag' ? (
                        <div className="grid max-w-lg grid-cols-3 gap-2">
                          {[
                            ...
                            <button key={value} type="button" onClick={() => updateResponse(question.id, value)} className={cn('rounded-lg border px-4 py-2.5 text-xs font-bold transition', responses[question.id] === value ? activeClass : `border-transparent ${idleClass}`)}>
      ...
      <footer className="flex shrink-0 flex-col-reverse gap-2 border-t border-background-200 bg-background-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
        ...
        <button type="button" onClick={submit} disabled={busy} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary-600 px-5 text-xs font-bold text-white shadow-sm transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-60">
