...
const MY_TICKETS = [
  { id: 'TKT-0892', subject: 'Learner unable to submit evidence — file upload failing', requester: 'Emily Watson', role: 'Learner', tenant: 'KBC', priority: 'high', status: 'open', category: 'Technical', created: '10 Jun 2026 14:22', sla: '1h 38m', description: 'User reports that when attempting to upload PDF evidence files, the upload spinner appears briefly then disappears with no file attached. Tried Chrome and Firefox on Windows 11. Browser console shows CORS error on upload endpoint.' },
  { id: 'TKT-0889', subject: 'OTJH hours not accumulating correctly for Cohort B learners', requester: 'James Okonkwo', role: 'Learner', tenant: 'KBC', priority: 'medium', status: 'in-progress', category: 'Data Issue', created: '10 Jun 2026 09:30', sla: '5h 30m', description: 'Multiple Cohort B learners report that OTJH hours logged this week show as 0 in the monthly total. Individual entries are visible but not summing. Suspect aggregation cron job.' },
  { id: 'TKT-0886', subject: 'WhatsApp notification templates not sending to Learner app', requester: 'Engagement Team', role: 'Engagement Manager', tenant: 'KBC', priority: 'medium', status: 'in-progress', category: 'Notifications', created: '9 Jun 2026 14:10', sla: '20h 50m', description: 'WhatsApp channel configured correctly but test messages not delivering. Email and SMS channels working fine. WhatsApp Business API logs show 401 on template sync.' },
  { id: 'TKT-0884', subject: 'Attendance mode "Self-Paced" not showing in dropdown for Cohort D', requester: 'David Osei', role: 'Coach', tenant: 'KBC', priority: 'low', status: 'resolved', category: 'Technical', created: '8 Jun 2026 15:30', sla: 'Resolved', description: 'Resolved — missing configuration in cohort settings. Added Self-Paced to Cohort D attendance modes.' },
  { id: 'TKT-0881', subject: 'Employer dashboard showing blank after login for KCC account', requester: 'Mark Johnson', role: 'Employer', tenant: 'KBC', priority: 'high', status: 'open', category: 'Bug Report', created: '7 Jun 2026 09:15', sla: '74h 45m', description: 'KCC employer account loads blank white page after successful login. Browser console shows React render error related to DashboardContext. Only happens on employer role.' },
  { id: 'TKT-0879', subject: 'Evidence file preview not loading for large PDFs (>20MB)', requester: 'Sarah Thompson', role: 'Coach', tenant: 'KBC', priority: 'medium', status: 'in-progress', category: 'Technical', created: '6 Jun 2026 16:40', sla: '42h 20m', description: 'PDF evidence files over 20MB show spinner indefinitely. Smaller files load fine. Need to implement chunked loading or server-side thumbnail generation.' },
  { id: 'TKT-0876', subject: 'Bulk learner import failed — CSV parsing error on row 47', requester: 'MIS Team', role: 'Super Admin', tenant: 'MAN', priority: 'high', status: 'open', category: 'Data Issue', created: '5 Jun 2026 14:00', sla: '93h 0m', description: 'Bulk import of 120 learners failed at row 47. CSV looks clean but parser throws "Unexpected token" error. Row 47 has a learner surname with apostrophe (O\'Brien).' },
  { id: 'TKT-0874', subject: 'Monthly coaching report email template missing learner name variable', requester: 'Priya Patel', role: 'QA Officer', tenant: 'LSA', priority: 'low', status: 'in-progress', category: 'Reporting', created: '4 Jun 2026 11:20', sla: '119h 40m', description: 'Monthly coaching report shows "Dear {LEARNER_NAME}" instead of actual name. Template variable not resolving. Check Handlebars rendering pipeline.' },
];

const STATUS_COLORS: Record<string, string> = {
  open: 'bg-red-50 text-red-700 border-red-200/50',
  'in-progress': 'bg-amber-50 text-amber-700 border-amber-200/50',
  escalated: 'bg-red-100 text-red-800 border-red-300/50',
...
const PRIORITY_COLORS: Record<string, string> = {
  ...
  low: 'bg-foreground-100 text-foreground-500',
};

export default function SupportMyTickets() {
  const [selectedTicket, setSelectedTicket] = useState<typeof MY_TICKETS[0] | null>(null);
  const [filterStatus, setFilterStatus] = useState('all');
  const [filterPriority, setFilterPriority] = useState('all');
  const [replyText, setReplyText] = useState('');
  const [replies, setReplies] = useState<Record<string, { text: string; time: string }[]>>({});

  const openCount = MY_TICKETS.filter(t => t.status === 'open').length;
  const inProgressCount = MY_TICKETS.filter(t => t.status === 'in-progress').length;
  const resolvedCount = MY_TICKETS.filter(t => t.status === 'resolved').length;

  const filtered = MY_TICKETS
  ...
  const handleSendReply = () => {
    ...
    setReplies(prev => ({
      ...prev,
      [selectedTicket.id]: [...(prev[selectedTicket.id] || []), { text: replyText, time: new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) }],
    }));
    setReplyText('');
  };
  ...
  return (
    <WorkspaceShell
      ...
      pageSubtitle={`${MY_TICKETS.length} tickets assigned to you · ${openCount} open · ${inProgressCount} in progress`}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={stat.label} className={`${stat.bg} rounded-xl p-4 text-center`}>
              ...
              <p className="text-[11px] text-foreground-500 mt-0.5">{stat.label}</p>
            </div>
          ))}
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap shrink-0 ${PRIORITY_COLORS[ticket.priority]}`}>
                        {ticket.priority}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_COLORS[ticket.status]}`}>
            ...
            {selectedTicket ? (
                    ...
                    {replies[selectedTicket.id].map((r, i) => (
                      <div key={i} className="bg-primary-50/50 rounded-lg p-2.5 border border-primary-100/50">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[10px] font-semibold text-primary-700">You</span>
              ...
              <div className="bg-background-50 rounded-xl border border-background-200/50 p-6 text-center">
                <div className="w-12 h-12 rounded-xl bg-background-100 flex items-center justify-center mx-auto mb-3">
                  <AppIcon className="ri-user-line text-foreground-300 text-xl"></AppIcon>
                </div>
                <p className="text-[13px] text-foreground-500">Select a ticket to view details</p>
                <p className="text-[11px] text-foreground-300 mt-1">These are all assigned to you</p>
              </div>
            )}
          </div>
        </div>
