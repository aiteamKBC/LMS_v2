...
export function WeeklyLearningPlan({ kind, learnerId, schedule, scheduleLoading, scheduleError }: {
  ...
  return <section aria-label="Weekly learning plan" className="grid grid-cols-1 gap-4 lg:items-stretch lg:grid-cols-[250px_minmax(0,1fr)]">
    ...
    <aside className="flex min-h-0 flex-col rounded-2xl border border-foreground-100 bg-background-50 p-4 shadow-sm">
      ...
      {weeks.length ? <>
      <ol className="mt-2.5 flex flex-1 flex-col">
        {visibleWeeks.map((week, index) => {
          ...
          return <li key={weekKey(week)} className="relative flex-1 pl-9">
      ...
      <PaginationControls page={weekPage} pageCount={weekPageCount} onPageChange={setWeekPage} label="weeks" className="mt-auto pt-3" />
    ...
    <div className="min-w-0 rounded-2xl border border-foreground-100 bg-background-50 p-4 shadow-sm lg:p-5">
