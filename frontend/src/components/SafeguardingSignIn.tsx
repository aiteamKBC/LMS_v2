...
const STORAGE_KEY = 'safeguarding_sso_pending';

/** Remembers the destination through password and Microsoft sign-in. */
export function SafeguardingSignIn({ children }: { children?: ReactNode }) {
  const { auth, isInitialized, initializationError } = useAuth();
  const location = useLocation();
  const redirecting = useRef(false);
  const queryState = new URLSearchParams(location.search).get('safeguarding_state');
  ...
  useEffect(() => {
    const state = new URLSearchParams(location.search).get('safeguarding_state');
    ...
    try {
      ...
      if (pending.expires > Date.now() && /^[a-f0-9]{64}$/.test(pending.state)) {
        ...
        window.location.replace(`/login_api/safeguarding/authorize/?state=${encodeURIComponent(pending.state)}`);
      }
    } catch { /* Ignore malformed browser state. */ }
